...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "BundleProcessNoWait-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name BundleProcessNoWait-service apps/BundleProcessNoWait-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app BundleProcessNoWait-service:1.0-SNAPSHOT
    oc expose svc/BundleProcessNoWait-service
else

	echo "Launching the application locally..."
	pattern="BundleProcessNoWait-service"
	files=( $pattern )
	cd ${files[0]}
